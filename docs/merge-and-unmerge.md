---
description: >-
  This page explains how to use merge and unmerge operations in MDMbox and how they
...
This page provides key information about using `$merge` and `$unmerge`. For full API details, refer to the [Swagger documentation](https://dev.mdm.health-samurai.io/backend/static/swagger.html).

{% hint style="success" %}
If you need **alternative merge and unmerge approaches** to adjust MDMbox to your specific workflows and requirements, please [contact us](https://health-samurai.io/contacts).
{% endhint %}
...
Before performing an unmerge, you should preview what will be changed using the **Unmerge Preview** endpoint. This endpoint returns a list of **encounters created after the merge**, which may need to be manually reassigned during the unmerge process.
...
- `encounters-and-patient-ids` — optional mapping of **encounters created after the merge** that need to be manually reassigned:
  ...
  - `encounter-ids` — list of encounters that should be re-linked to this record. This ensures that any encounters created post-merge are correctly redirected when restoring the original records. You can get the list of post-merge encounters via `$unmerge-preview` request.
